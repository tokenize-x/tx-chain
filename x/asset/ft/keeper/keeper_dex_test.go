...
// TestKeeper_FrozenTokenEscapeViaDEX verifies that frozen tokens cannot be
// DEX-locked or transferred via DEX settlement.
//
// If the fix were missing: DEXIncreaseLocked would allow locking frozen tokens
// (it only checked balance - dexLocked - bankLocked), and DEXExecuteActions would
// transfer them via raw bank keeper, bypassing FT hooks. This test asserts the
// fix: the second lock attempt (which would lock frozen tokens) must fail.
func TestKeeper_FrozenTokenEscapeViaDEX(t *testing.T) {
	requireT := require.New(t)
	...
	// ========================================================================
	// SETUP: Create token with Feature_freezing, fund Alice, freeze tokens
	// ========================================================================

	issuer := sdk.AccAddress(secp256k1.GenPrivKey().PubKey().Address())
	alice := sdk.AccAddress(secp256k1.GenPrivKey().PubKey().Address())
	bob := sdk.AccAddress(secp256k1.GenPrivKey().PubKey().Address())

	// Issue a token with the freezing feature enabled (the "base" token Alice sells)
	settings := types.IssueSettings{
		Issuer:        issuer,
		Symbol:        "VULN",
		Subunit:       "vuln",
	...
	// Issue a second token (the "quote" token Bob uses to buy)
	quoteSettings := types.IssueSettings{
		Issuer:        issuer,
		Symbol:        "QUOTE",
		Subunit:       "quote",
		Precision:     6,
	...
	quoteDenom, err := ftKeeper.Issue(ctx, quoteSettings)
	requireT.NoError(err)

	// Fund Alice with 100 base tokens
	aliceCoins := sdk.NewInt64Coin(denom, 100)
	requireT.NoError(bankKeeper.SendCoins(ctx, issuer, alice, sdk.NewCoins(aliceCoins)))

	// Fund Bob with 100 quote tokens (for buying)
	bobCoins := sdk.NewInt64Coin(quoteDenom, 100)
	requireT.NoError(bankKeeper.SendCoins(ctx, issuer, bob, sdk.NewCoins(bobCoins)))

	// Admin freezes 60 of Alice's base tokens
	freezeAmount := sdk.NewInt64Coin(denom, 60)
	requireT.NoError(ftKeeper.Freeze(ctx, issuer, alice, freezeAmount))

	// Verify initial state
	balance := bankKeeper.GetBalance(ctx, alice, denom)
	requireT.Equal(int64(100), balance.Amount.Int64(), "Alice should have 100 tokens")

	frozenBalance, err := ftKeeper.GetFrozenBalance(ctx, alice, denom)
	requireT.NoError(err)
	requireT.Equal(int64(60), frozenBalance.Amount.Int64(), "Alice should have 60 frozen tokens")

	spendable, err := ftKeeper.GetSpendableBalance(ctx, alice, denom)
	requireT.NoError(err)
	requireT.Equal(int64(40), spendable.Amount.Int64(), "Alice should have 40 spendable tokens")

	t.Log("=== Initial state: balance=100, frozen=60, dexLocked=0, spendable=40 ===")

	// ========================================================================
	// PHASE 1: Verify that frozen tokens cannot be DEX-locked
	//
	// If the vulnerability existed: DEXIncreaseLocked would only check
	// (balance - dexLocked - bankLocked >= amount) without considering frozen.
	// That would allow locking 40 more tokens (overlapping with frozen), then
	// 20 more, until all 60 frozen tokens were DEX-locked and transferable.
	// The fix: validateCoinIsNotLockedByDEXAndBank now subtracts frozen from
	// available balance, so only truly spendable tokens can be DEX-locked.
	// ========================================================================

	// Step 1: Lock 40 tokens (the unfrozen portion) — must succeed
	lockAmount1 := sdk.NewInt64Coin(denom, 40)
	err = ftKeeper.DEXIncreaseLocked(ctx, alice, lockAmount1)
	requireT.NoError(err, "First lock of 40 should pass (40 unfrozen available)")

	dexLocked := ftKeeper.GetDEXLockedBalance(ctx, alice, denom)
	requireT.Equal(int64(40), dexLocked.Amount.Int64(), "40 tokens should be DEX-locked")
	t.Log("Step 1: DEXIncreaseLocked(40) passed. dexLocked=40")

	// Step 2: Lock 40 MORE tokens — must FAIL (would lock frozen tokens)
	// Available spendable = 100 - 40 (dexLocked) - 60 (frozen) = 0.
	// If the fix were missing, (balance - dexLocked) = 60 >= 40 would incorrectly pass.
	lockAmount2 := sdk.NewInt64Coin(denom, 40)
	err = ftKeeper.DEXIncreaseLocked(ctx, alice, lockAmount2)
	requireT.Error(err, "Second lock of 40 must fail: it would lock frozen tokens; available spendable is 0")
	t.Log("Step 2: DEXIncreaseLocked(40) correctly rejected — frozen tokens cannot be DEX-locked")

	// Verify state unchanged: still 40 DEX-locked, 60 frozen
	dexLocked = ftKeeper.GetDEXLockedBalance(ctx, alice, denom)
	requireT.Equal(int64(40), dexLocked.Amount.Int64(), "DEX-locked should remain 40")
	frozenBalance, err = ftKeeper.GetFrozenBalance(ctx, alice, denom)
	requireT.NoError(err)
	requireT.Equal(int64(60), frozenBalance.Amount.Int64(), "Frozen should remain 60")
}
