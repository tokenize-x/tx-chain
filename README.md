...
After the command completes you may find executable `./bin/txd`, being both blockchain node and client.
...
After a while 3 `txd` validators will be deployed to your docker. 
...
```
(znet) [znet] $ txd-00-val keys list
```
...
```
(znet) [znet] $ txd-00-val q bank balances {YOUR_GENERATED_ADDRESS}
```
...
```
(znet) [znet] $ txd-00-val tx bank send alice {YOUR_GENERATED_ADDRESS} 10udevcore --broadcast-mode=block
```
...
You can also find block explorers for each chain by this <!-- markdown-link-check-disable -->[link](https://docs.tx.org/docs/tools-and-ecosystem/blockchain-explorers)<!-- markdown-link-check-enable -->.
