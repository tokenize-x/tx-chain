...
// on TX and calling it from Osmosis.
func TestIBCHooksCounterWASMCall(t *testing.T) {
	...
	sendToOsmosisCoin := txChain.NewCoin(sdkmath.NewInt(10_000))
	...
	sendToTXCoin := sdk.NewCoin(
		expectedOsmosisRecipientBalance.Denom,
		expectedOsmosisRecipientBalance.Amount.Quo(sdkmath.NewInt(2)),
	)

	sendOsmosisToTXCoin := osmosisChain.NewCoin(sdkmath.NewInt(10_000))
	expectedOsmosisOnTXBalance := sdk.NewCoin(
		ConvertToIBCDenom(txToOsmosisChannelID, sendOsmosisToTXCoin.Denom),
		sendOsmosisToTXCoin.Amount,
	)

	ibcHookCallerOnTxAddr1, err := ibchookskeeper.DeriveIntermediateSender(
		txToOsmosisChannelID,
		osmosisChain.MustConvertToBech32Address(osmosisHookCaller1),
		txChain.ChainSettings.AddressPrefix)
	requireT.NoError(err)

	ibcHookCallerOnTxAddr2, err := ibchookskeeper.DeriveIntermediateSender(
		txToOsmosisChannelID,
	...
	requireT.NotEqual(ibcHookCallerOnTxAddr1, ibcHookCallerOnTxAddr2)

	// Osmosis tx memo contains info to call WASM contract on TX this info is propagated into
	...
	ibcHookMemo := fmt.Sprintf(`{"wasm":{"contract": "%s", "msg":{"increment":{}}}}`, txContractAddr)
	...
	awaitCounterContractState(
		...
		txChain,
		txContractAddr,
		ibcHookCallerOnTxAddr1,
		0,
		sdk.Coins{txChain.NewCoin(sendToTXCoin.Amount)},
	)
	...
	awaitCounterContractState(
		...
		ibcHookCallerOnTxAddr1,
		1,
		sdk.Coins{txChain.NewCoin(sendToTXCoin.Amount.Add(sendToTXCoin.Amount))},
	)
	...
	awaitCounterContractState(
		...
		txChain,
		txContractAddr,
		ibcHookCallerOnTxAddr2,
		0,
		sdk.Coins{expectedOsmosisOnTXBalance},
	)
}

// TestIBCHooksCounterWASMCallback tests ibc-hooks integration by deploying the ibc-hooks-counter WASM contract
// on TX and using it as a callback for IBC transfer sent to Osmosis.
func TestIBCHooksCounterWASMCallback(t *testing.T) {
	...
	ibcCallbackMemo := fmt.Sprintf(`{"ibc_callback": "%s"}`, txContractAddr)
	sendToOsmosisCoin := txChain.NewCoin(sdkmath.NewInt(10_000))
	_, err = txChain.ExecuteIBCTransferWithMemo(
		ctx,
		t,
		txChain.TxFactory().WithGas(txChain.GasLimitByMsgs(&ibctransfertypes.MsgTransfer{})),
		txSender,
...
func awaitCounterContractState(
	...
	require.NoError(t, retry.Do(retryCtx, time.Second, func() error {
		...
		queryCountOut, err := txChain.Wasm.QueryWASMContract(ctx, contractAddr, getCountPayload)
		if err != nil {
		...
		require.NoError(t, err)
		queryTotalFundsOut, err := txChain.Wasm.QueryWASMContract(ctx, contractAddr, getTotalFundsPayload)
		if err != nil {
			if strings.Contains(err.Error(), "not found") {
				return retry.Retryable(errors.New("counter is still not found for address: " + callerAddr))
			}
